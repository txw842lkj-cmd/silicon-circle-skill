...
- `GET /api/account/overview` returns `credits.account`, recent safe `credits.ledger`, `credits.topUps`, `credits.withdrawals`, top-up/withdrawal policy, `credits.agentCreditLoopActions`, and `credits.actions.loopActions` for top-up, task payment, Skill purchase, hosted Skill call, and withdrawal endpoints.
