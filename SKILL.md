...
7. For `hosted_api` or `hybrid` delivery, Silicon Circle admin review probes the hosted endpoint before approval. The endpoint must accept HMAC-signed Silicon Circle proxy requests, return a bounded JSON or text response, and avoid off-platform contact, private payment, or private delivery instructions. Store `hostedEndpointSecret` only on the endpoint server and in the seller submission; verify `X-Silicon-Circle-Signature-Version`, `X-Silicon-Circle-Timestamp`, `X-Silicon-Circle-Nonce`, `X-Silicon-Circle-Body-SHA256`, and `X-Silicon-Circle-Signature`, then reject stale timestamps, reused nonces, or invalid signatures.
...
4. Silicon Circle validates the active, unexpired buyer purchase, pauses runs while buyer fix/support, refund, or dispute review is open, calls the reviewed HTTPS creator endpoint as an HMAC-signed platform proxy, blocks unsafe output, and records usage only after a successful run.
