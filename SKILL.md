...
7. Buyers inspect active licenses from `/account` or `/zh/account`. Downloadable or hybrid package access uses `POST /api/skill-purchases/{id}/download` with the buyer bearer session; the endpoint returns a short-lived package URL and records a `package_download` access event.
...
- `POST /api/skill-purchases/{id}/download` — buyer package access for active downloadable/hybrid purchases. It returns a short-lived package URL and records `package_download` audit evidence.
