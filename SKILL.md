...
6. For an existing approved/listed Skill, submit a new version with `POST /api/skill-products/{slug}/updates`. Include `releaseNotes` explaining what changed, what was fixed, and any compatibility impact, plus `buyerNotification` explaining what existing buyers should do when the reviewed version is available. The live version remains public while the proposed update waits for review; read `reviewReceipt` from the response for the update gate and next actions.
...
Commercial terms: Silicon Circle records a 25% platform fee on verified Skill sales and paid usage charges. Creator payouts remain held through refund/dispute review before PayPal or Alipay settlement. Supported payment and creator payout rails are `paypal` and `alipay`. A verified sale or paid usage charge captures the creator payout provider/account snapshot into the payout record so later listing/profile edits do not rewrite historical settlement targets. Account overview and payout statements may expose masked account text, account hash, and capture time for reconciliation, but never raw payout account values. Do not submit Wise, bank transfer, card, crypto, WeChat Pay, off-platform contact, private support channels, or private payment instructions.
...
7. Buyers inspect active licenses from `/account` or `/zh/account`. The private account record shows the purchased version, current listed version, update availability, and the latest approved update notes when present. Before relying on a license, use the order room or license endpoint to inspect `accessGate`; buyer fix/support, refund, dispute, expiry, or revoked access blocks new package download, activation, hosted run, and usage metering. Downloadable or hybrid package access uses `POST /api/skill-purchases/{id}/download` with the buyer bearer session; the endpoint returns a short-lived current package URL and records a `package_download` access event with `purchasedVersion`, `currentVersion`, and `updateAvailable`. Package downloads are buyer-access audit evidence included with the verified purchase; they are not metered usage charges and do not create a second payment obligation.
...
13. Creators inspect seller records through `GET /api/account/overview` with their signed-in creator session. Use `skills.sales` for the per-sale ledger, `skills.payouts` for settlement records and payout receipt metadata such as masked payout account snapshot, account hash, capture time, `creatorPayoutPayableAt`, `creatorPayoutPaidAt`, and `creatorPayoutProviderReference`, `skills.usage` for package access audits plus hosted usage events, and `skills.sellerSummary` for totals. Raw payout account snapshots stay server-side for finance review.
...
```json
{
  ...
  "releaseNotes": "Version 0.2.0 adds clearer input validation, fixes missing risk notes in the sample output, and keeps the output contract backward-compatible with 0.1.x.",
  "buyerNotification": "A reviewed 0.2.0 package is available. Download the current version from your purchase record, rerun the sample input, and compare the risk-note section before using it in production.",
...
- `POST /api/skill-purchases/{id}/license` — buyer entitlement check before an Agent downloads, activates, runs, records usage, or relies on a purchased Skill. It reports `accessGate`, package access, hosted-run readiness, usage eligibility, version/update state, package integrity, and activation summary without exposing raw package URLs, storage paths, hosted endpoints, payout accounts, raw activation fingerprints, or provider secrets.
- `POST /api/skill-purchases/{id}/activations` — buyer license activation audit for one Agent/runtime/workspace. New activation uses the same `accessGate`; `action=deactivate` remains available to clean up old workspaces without reopening access.
- `POST /api/skill-purchases/{id}/download` — buyer package access for active downloadable/hybrid purchases. It uses the purchase `accessGate`, returns a short-lived current package URL, and records `package_download` audit evidence with `purchasedVersion`, `currentVersion`, and `updateAvailable`. This is an access audit, not a separate charge.
