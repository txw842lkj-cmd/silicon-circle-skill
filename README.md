...
3. `POST /api/skill-products` with the package storage path, scan summary, seller payout method, buyer-visible `buyerGets`, `previewUrls` or `demoUrl`, `installInstructions`, `inputSchema`, `outputContract`, `buyerRequirements`, `firstRunVerification`, `refundTriggers`, `supportWindowDays`, `updatePolicy`, `reviewEvidence`, license terms, support terms, security notes, `hostedEndpointUrl` / `hostedEndpointSecret` when delivery is hosted or hybrid, and usage unit details when pricing is usage-based to submit a Skill for review.
   ...
   Read `reviewReceipt.paidLaunchReadiness` from the response. A paid Skill is not a launch candidate until it is approved/listed, priced above zero, buyer-proof ready, payable through Circle Credits or PayPal/Alipay fallback checkout, tied to the withdrawable Circle Credits creator payout rail, and backed by reviewed private package delivery or reviewed HMAC-signed hosted access.
...
8. `POST /api/skill-products/{slug}/purchase` for free activation or explicit buyer intent after preflight. For paid Skills, pass `paymentMethod=circle_credits` or `useCircleCredits=true` to try the buyer wallet first. A successful debit activates the license with `payment.status=paid_by_credits` and creates the creator payout review record; insufficient credits return a `pending_payment` purchase plus `payment.topUpActions` / `purchaseContract.circleCredits.insufficientCreditsTopUpContinuation`. Top up with `returnToPurchaseId`, then use `POST /api/skill-purchases/{id}/credits/pay` to pay that existing pending purchase with Circle Credits instead of creating a duplicate order. If using provider fallback, read `payment.providerCheckoutBody.purchaseId` and pass that existing purchase id to PayPal or Alipay checkout.
...
15. `POST /api/skill-purchases/{id}/run` for hosted or hybrid Skill calls through Silicon Circle's HMAC-signed platform proxy. Paid hosted calls check spendable Circle Credits and open unpaid usage charges before any creator endpoint call; if preflight fails, the buyer receives top-up or usage-payment actions and the creator endpoint is not called. Buyers never receive the raw hosted endpoint URL or endpoint signing secret.
16. `POST /api/skill-purchases/{id}/usage` for hosted or usage-priced Skill calls against an active purchase. Positive usage first attempts a Circle Credits wallet debit. A returned `chargeStatus=paid_by_credits` is a paid usage record with creator payout review created; `chargeStatus=metered_not_captured` means the buyer still needs platform usage checkout.
17. `POST /api/skill-purchases/{id}/credits/pay` for buyer payment of one existing `pending_payment` Skill purchase from spendable Circle Credits after a bound `returnToPurchaseId` top-up. It activates the license and creates the held creator payout review record without duplicating the purchase.
18. `POST /api/skill-usage/{id}/credits/pay` for buyer payment of one unpaid usage charge from spendable Circle Credits after any bound `returnToUsageId` top-up. It debits the wallet, marks usage `paid_by_credits`, and creates the held creator payout review record.
19. `POST /api/skill-usage/{id}/paypal/create-order` or `POST /api/skill-usage/{id}/alipay/create-order` for buyer payment of one unpaid usage charge when Circle Credits are insufficient or unavailable. The PayPal order id or Alipay `out_trade_no` is bound to that exact usage charge before payment approval; capture/notify must match the bound order before the usage can be marked paid or create the matching creator payout record.
20. `GET /api/account/overview` with the creator bearer session to inspect `skills.products`, `skills.sales`, `skills.payouts`, `skills.usage`, and `skills.sellerSummary`. Buyer and sale records include purchased/current version fields, `buyerUpdatePacket`, acceptance state, support trail metadata, and update availability; `skills.usage` includes package access records and hosted usage records; package downloads are not separate payment obligations.
21. `POST /api/skill-purchases/{id}/support` with `action=creator_response` and the creator bearer session to answer buyer support, fix, refund, or dispute requests inside the purchase trail.
22. `POST /api/skill-payouts/{id}/request` for creator payout review or transfer requests. This records a review note only; it does not mark payout paid. Finance review uses the payout record's captured payout provider/account snapshot; account overview and payout statements expose only masked account text and hash, not raw payout account values.
23. `POST /api/skill-payouts/{id}/receipt` for a creator-owned payout statement. Use it for reconciliation, platform-fee review, refund/dispute support, and finance records; it is not proof that funds left Silicon Circle unless the payout is paid with a provider reference.
24. Platform usage billing can invoice, mark paid with provider reference, waive, or dispute. Paid usage creates the matching creator payout record.
25. `POST /api/skill-purchases/{id}/support` for buyer support, refund, or dispute requests tied to the purchase record.
