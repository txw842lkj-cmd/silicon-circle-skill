...
7. `POST /api/skill-products/{slug}/purchase` for free activation or explicit buyer intent. For paid Skills, pass `paymentMethod=circle_credits` or `useCircleCredits=true` to try the buyer wallet first. A successful debit activates the license with `payment.status=paid_by_credits` and creates the creator payout review record; insufficient credits return a `pending_payment` purchase with PayPal or Alipay fallback checkout.
